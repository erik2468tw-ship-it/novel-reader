    ...
    for (let retry = 0; retry < MAX_RETRIES; retry++) {
        try {
            ...
            try {
                ...
                if (titleEl) title = await titleEl.innerText();
            } catch (e) {}
            if (!title || title === '未知章節') {
            ...
            content = content.replace(/[\r\n]+/g, '\n').replace(/\t/g, '').replace(/ {2,}/g, ' ')
                ...
                .reduce((acc, line, i, arr) => {
                    if (i > 0 && line.length < 10 && arr[i-1].length < 100) acc[acc.length - 1] = acc[acc.length - 1] + ' ' + line;
                    else acc.push(line);
                    return acc;
                }, []).join('\n\n').trim();
...
async function scrapeNovel(novel, retryCount = 0) {
    ...
    try {
        ...
        for (let i = existingCount; i < chapters.length; i++) {
            ...
            if (data && data.content) { saveChapterToFile(novel.novelId, i + 1, data.title, data.content); newChapters++; }
        ...
        if (failedChapters.length > 0 && retryCount < MAX_DOWNLOAD_RETRIES) {
            ...
            for (const failed of failedChapters) {
                if (failed.retry < MAX_RETRIES) {
                    ...
                    if (data && data.content) { saveChapterToFile(novel.novelId, failed.index + 1, data.title, data.content); newChapters++; }
                    else failed.retry++;
                }
            }
        }
